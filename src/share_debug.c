...
double *deviance,double *tol,long *verbose, long *phase, long *Minherit)
{
  ...
  **/
     ...
     double *train_wgt, *test_wgt, **train_ngcov=NULL, **test_ngcov=NULL, obssum, wgtsum, meany, temp_phi, min_phi=0, *mustart, **dev_mat, min_freq, **ngcov_mat=NULL;
     hmodel *temp, *base_model=NULL, *best_model=NULL; 
     Node SNP_out;
...
// here count1 is observations, count2 is subjects
     ...
     nobs_train=(long *) Calloc(1,long);//malloc(sizeof(long));
...
// Prepare the linked list of all snps to start the search 
         ...
         for (j=0;j<*nloci;j++) {
                  next_snp = Calloc(1,Node);//malloc(sizeof(Link1));
...
long *maxsnps, long *bestsize, long *output_snp_set, long *out_haplo_vec, double *out_hap_freq, double *tol, long *phase, double *varstore, double *coef, long *out_nhaps, long *Minherit, long *verbose){
 ...
 double *mustart, min_phi=0, temp_phi, min_freq, **ngcov_mat=NULL;  
 hmodel *temp, *base_model=NULL, *best_model=NULL; 
 Node SNP_out;
 Link1 current_snp= &SNP_out, next_snp;   
...
long Minherit, long *verbose,FILE *file, long *phase, double *varstore){
   ...
   double *out_hfreq, **amat, **bmat=NULL, *varmu, *escore=NULL; 
   double **x, temp_freq, *pred, *err;

   out_nhaps=(long *) Calloc(1,long);//malloc(sizeof(long));
        ...
        FILE *file)                  
{
   ...
   deviance=(double *) Calloc(1,double);//malloc(sizeof(double));
   out_nhaps=(long *) Calloc(1,long);//malloc(sizeof(long));
...

{
   ...
   double **x,  *pred, *outcome=NULL, *wgt=NULL, *wgtsum, dev_out, mindist, *dist_hap, *locus_freq;
...
void stepwise_search_alpha(long *indx_subj,
  ...
  double *mustart, min_phi=0, ncase, bestdev, temp_phi, mindev, min_freq,meany, **ngcov_mat=NULL;  
  hmodel *temp, *best_model=NULL, *base_model=NULL; 
  Node SNP_out;
