    ...
    if (ModSelMethod=="Cross-Val") {
        ...
        return(fSubset)
        		...
        		if (outObj@bestsize==0) {
        			...
        			pval <- drop(
		  			  1-stats::pchisq(t(temp$coef[2:(temp$outnhaps+temp$nngcov)]) %*% MASS::ginv(var[2:(temp$outnhaps+temp$nngcov),2:(temp$outnhaps+temp$nngcov)]) %*% temp$coef[2:(temp$outnhaps+temp$nngcov)],(temp$outnhaps+temp$nngcov)-1))
