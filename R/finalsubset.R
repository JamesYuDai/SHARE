...
finalsubset <- function(shareObj,
    ...
    if (shareObj@bestsize==0) {
        ...
        var<-MASS::ginv(amat)%*% bmat %*% MASS::ginv(amat)
        
        pval <- drop(
			1-stats::pchisq(t(temp1$coef[2:(temp1$outnhaps+temp1$nngcov)]) %*% MASS::ginv(var[2:(temp1$outnhaps+temp1$nngcov),2:(temp1$outnhaps+temp1$nngcov)]) %*% temp1$coef[2:(temp1$outnhaps+temp1$nngcov)],(temp1$outnhaps+temp1$nngcov)-1))
        ...
        ## p-values
        pValue <- 1-stats::pnorm(abs(zScore))
